...
title_search = re.compile(r"""
  (title:\s*) # look for 'title:' followed by any number of spaces
  (?P<title>  # group the below with group name 'title'
    (         # this group matches every text line that's not an empty line
      (.*     # matches every char
      \S+)    # followed by at least one non-white char
    \n)*      # followed by a line
  )""", 
